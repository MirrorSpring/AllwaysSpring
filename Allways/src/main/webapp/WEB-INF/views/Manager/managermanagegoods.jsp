...
<form action="addGoods.do" method="post" enctype="multipart/form-data" name="actionForm">
	...
	<div style="display: inline-block;width:60%">
		...
		<div align="left" style="width:74%;display:inline-block">
			<div style="width:50%;display:inline-block">
				...
				<c:if test="${check==null }">
					<input type="text" name="goodsName" class="form-control" style="border-color:#fdcdcd" value="${dto.goodsName }">
				</c:if>
				<c:if test="${check==true }">
					<script>swal("사용 가능한 이름입니다.",'','success');</script>
					<input type="text" name="goodsName" class="form-control is-valid" style="border-color:#fdcdcd" value="${dto.goodsName }">
				</c:if>
				<c:if test="${check==false }">
					<script>swal("이미 사용중인 이름입니다.",'','warning');</script>
					<input type="text" name="goodsName" class="form-control is-invalid" style="border-color:#fdcdcd" value="${dto.goodsName }">
				</c:if>
			</div>
		...
		<div align="left" style="width:74%;display:inline-block">
			<div style="width:30%;display:inline-block">
				<input type="number" step="1000" name="goodsPrice" class="form-control" style="border-color:#fdcdcd" value="${dto.goodsPrice }">
			</div>원
		</div>
		...
		<div align="left" style="width:74%;height:20%;display:inline-block">
			<textarea style="width:100%;margin-top:20px;border-color:#fdcdcd" class="form-control" name="goodsDetail">${dto.goodsDetail }</textarea>
		</div>
		<div align="left" style="width:20%;display:inline-block">
			사진
		</div>
